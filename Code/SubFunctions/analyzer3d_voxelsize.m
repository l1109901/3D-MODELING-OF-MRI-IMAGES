function varargout = analyzer3d_voxelsize(varargin)
% analyzer3d_VOXELSIZE M-file for analyzer3d_voxelsize.fig
%      analyzer3d_VOXELSIZE, by itself, creates a new analyzer3d_VOXELSIZE or raises the existing
%      singleton*.
%
%      H = analyzer3d_VOXELSIZE returns the handle to a new analyzer3d_VOXELSIZE or the handle to
%      the existing singleton*.
%
%      analyzer3d_VOXELSIZE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in analyzer3d_VOXELSIZE.M with the given input arguments.
%
%      analyzer3d_VOXELSIZE('Property','Value',...) creates a new analyzer3d_VOXELSIZE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before analyzer3d_voxelsize_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to analyzer3d_voxelsize_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help analyzer3d_voxelsize
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @analyzer3d_voxelsize_OpeningFcn, ...
                   'gui_OutputFcn',  @analyzer3d_voxelsize_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before analyzer3d_voxelsize is made visible.
function analyzer3d_voxelsize_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to analyzer3d_voxelsize (see VARARGIN)

% Choose default command line output for analyzer3d_voxelsize
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes analyzer3d_voxelsize wait for user response (see UIRESUME)
% uiwait(handles.figurevoxelsize);


% --- Outputs from this function are returned to the command line.
function varargout = analyzer3d_voxelsize_OutputFcn(hObject, eventdata, handles) 
