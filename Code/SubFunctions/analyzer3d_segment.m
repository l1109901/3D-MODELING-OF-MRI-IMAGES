function varargout = analyzer3d_segment(varargin)
% analyzer3d_SEGMENT MATLAB code for analyzer3d_segment.fig
%      analyzer3d_SEGMENT, by itself, creates a new analyzer3d_SEGMENT or raises the existing
%      singleton*.
%
%      H = analyzer3d_SEGMENT returns the handle to a new analyzer3d_SEGMENT or the handle to
%      the existing singleton*.
%
%      analyzer3d_SEGMENT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in analyzer3d_SEGMENT.M with the given input arguments.
%
%      analyzer3d_SEGMENT('Property','Value',...) creates a new analyzer3d_SEGMENT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before analyzer3d_segment_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to analyzer3d_segment_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help analyzer3d_segment

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @analyzer3d_segment_OpeningFcn, ...
                   'gui_OutputFcn',  @analyzer3d_segment_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before analyzer3d_segment is made visible.
function analyzer3d_segment_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to analyzer3d_segment (see VARARGIN)

% Choose default command line output for analyzer3d_segment
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes analyzer3d_segment wait for user response (see UIRESUME)
...
n=structfind(data.substorage,'name','analyzer3d_segment');
if(isempty(n))
   n=length(data.substorage)+1;
   data.substorage(n).name='analyzer3d_segment';
...
function varargout = analyzer3d_segment_OutputFcn(hObject, eventdata, handles) 
...
analyzer3d('show3d_Callback',gcf,[false false],guidata(gcf));
...
setMyData(data);
analyzer3d('UpdatedVolume_Callback',gcf,dvs2,guidata(gcf));
